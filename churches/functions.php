<?php
    ...
    function getPosts($church)
    {
      ...
      global $conn;
      $query = $conn->query("SELECT *, (SELECT COUNT(*) FROM posts_like WHERE postId = posts.id) as nlikes, (SELECT COUNT(*) FROM posts_comments  WHERE postId = posts.id) as ncomments FROM posts JOIN users ON posts.postChurchAdmin = users.Id  WHERE users.church = \"$church\" ORDER BY postedDate DESC ") or trigger_error($conn->error);

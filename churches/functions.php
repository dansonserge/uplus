<?php
		...
		function getPosts($church)
		{
			...
			global $conn;
			$sql = "SELECT *, posts.type as type, (SELECT COUNT(*) FROM posts_like WHERE postId = posts.id) as nlikes, (SELECT COUNT(*) FROM posts_comments  WHERE postId = posts.id) as ncomments FROM posts JOIN users ON posts.postChurchAdmin = users.Id  WHERE users.church = \"$church\" AND (ISNULL(posts.archivedDate) OR posts.archivedDate <= '0000-00-00 00:00:00' ) ORDER BY postedDate DESC ";

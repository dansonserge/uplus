<?php
    ...
    function checkType($user){
      ...
      //checks user type
      $query = $db->query("SELECT type FROM users WHERE Id = \"$user\" ");
      if($query){
    ...
    function checkLogin($userType){
      ...
      //checking the type
      $query = $db->query("SELECT * FROM users WHERE Id = \"$user\" LIMIT 1 ") or die("usercheck error $db->error");
      $userData = $query->fetch_assoc();
    ...
    function church_event($churchID){
      global $db;
      $events = array();
      $query = $db->query("SELECT * FROM event WHERE church = \"$churchID\" ORDER BY eventStart DESC LIMIT 20") or die("$db->error");
    ...
    function member_types($church = ''){
      ...
      $query = $db->query("SELECT * FROM member_types") or die("Can't get mtypes $db->error");
      $types = array();
    ...
    function group_types($church = ""){
      ...
      if($church){
      	...
      	$query = $db->query($sql) or die("Error $db->error");
	    ...
	    while ($data = $query->fetch_assoc()) {
      	...
      	$query = $db->query("SELECT * FROM group_types ORDER BY `group_types`.`drank` ASC") or die("Can't get gtypes $db->error");
      	$types = array();
    ...
    function group_details($group){
      //group details
      global $db;
      $query = $db->query("SELECT * FROM groups WHERE groups.id = \"$group\" LIMIT 1");
      if($query->num_rows > 0){
        $group_data = $query->fetch_assoc();
        //getting number of members
        $members = $db->query("SELECT COUNT(*) as num FROM group_members WHERE groupid = $group_data[id]");
    ...
    function churchAdmin($churchID){
    	...
    	$query = $db->query("SELECT *, CONCAT(fname, ' ', lname)  as name FROM users WHERE type = 'church' AND church = \"$churchID\" LIMIT 1 ") or trigger_error($db->error);
    ...
    function church_forums(){
    	//returns church
    	global $db;
    	$query = $db->query("SELECT * FROM forums ") or trigger_error("can't  get forums $db->error");
    ...
    function group_members($group){
      //members details
      global $db;
      $query = $db->query("SELECT * FROM group_members as gm JOIN members as m ON gm.member = m.id WHERE gm.groupid = \"$group\"") or die("Can;t get group members $db->error");
      $members = array();
      while ($data = $query->fetch_assoc()) {
        $members[] = $data;
      }
    ...
    function branch_leader($branch, $position=''){
      ...
      $query = $db->query("SELECT * FROM users JOIN branch_leaders ON users.Id = branch_leaders.user WHERE branch_leaders.branch = \"$branch\" AND position LIKE  \"%$position%\" ") or die("Error getting branch staff");
      $leaders = array();
      while ($data = $query->fetch_assoc()) {
        $leaders[] = $data;
      }
    ...
    function church_branches($church){
  		//Getting branches
  		global $db;
  		$branchesQuery = $db->query(  "SELECT * FROM branches WHERE church = $church ") or die("Can't get branches ".$db->error);
  		$branches = array();
    ...
    function church_donations($church){
      ...
      $query = $db->query("SELECT *, members.name as membername, donations.id as donation_id, donations.date as donation_date, branches.name as branchname FROM donations JOIN members ON donations.member = members.id JOIN branches ON members.branchid = branches.id WHERE branches.church  = \"$church\" ") or die("Can't get donations $db->error");
      $donations = array();
    ...
    function church_services($church, $branch = ''){
      ...
      if(empty($branch)){
        ...
        $query = $db->query($sql) or trigger_error("Can't get cservices $db->error");
    ...
    function get_branch($branch){
      ...
      global $db;
      $query = $db->query("SELECT * FROM branches WHERE branches.id = \"$branch\" LIMIT 1 ") or die("Can't get branch $db->error");
      return $query->fetch_assoc();
    }
    ...
    function donation_sources($church){
      ...
      $query = $db->query($sql) or die("Can't get the donation sources $db->error");
      $sources = array();
    ...
    function service_details($service_id){
      //Function to get user's details
      global $db;
      $service = $db->query("SELECT * FROM service WHERE id = \"$service_id\" LIMIT 1 ") or die("Errror getting sevice's details $db->error");
    ...
    function donations_by_service($church){
      ...
      $query = $db->query($sql) or die("Errror getting donations service status $db->error");
    ...
    function group_non_members($group){
      //members details
      global $db;
      $query = $db->query("SELECT * FROM members WHERE members.id NOT IN (SELECT member FROM group_members WHERE group_members.groupid = \"$group\") ") or die("Can;t get group non-members $db->error");
      $members = array();
      while ($data = $query->fetch_assoc()) {
        $members[] = $data;
      }
    ...
    function smsbalance($churchID){
      ...
      $query = $db->query("SELECT * FROM smsbalance WHERE church = \"$churchID\" LIMIT 1 ") or die("Cant get the balance $db->error");
    ...
    function churchSMSname($churchID){
      ...
      $query = $db->query("SELECT COALESCE(smsName, name) smsName FROM church WHERE id = \"$churchID\" LIMIT 1 ") or die("Cant get the sms name $db->error");
      $data = $query->fetch_assoc();
    ...
    function msend($logID){
        ...
        $det = $db->query("SELECT email, subject, token, sender, smsName, members.phone as receiver, message.message as message, channel FROM messageslog JOIN message ON messageslog.message = message.id JOIN members ON messageslog.receiver = members.id WHERE messageslog.id = \"$logID\" LIMIT 1 ") or trigger_error("can get log data ".mysqli_error($db));
        ...
        if(mysqli_num_rows($det)){
            ...
            if($smsstatus == "Yes")
            {
                $query = $db->query("UPDATE messageslog SET status='sent' WHERE id = \"$logID\"") or die(mysqli_error($db));
                return true;
            }
            elseif($smsstatus == "No")
            {
                $query = $db->query("UPDATE messageslog SET status='failed' WHERE id = \"$logID\"") or die(mysqli_error($db));
                return false;
            }
        }
        else
    ...
    function list_groups($churchID){
      global $db;
      $query = $db->query("SELECT groups.*, branches.name as branchname FROM groups JOIN branches ON groups.branchid = branches.id WHERE branches.church = \"$churchID\" ") or die("Cant get groups ".$db->error);
    ...
    function branch_groups($branch){
      global $db;
      $query = $db->query("SELECT * FROM groups WHERE branchid = \"$branch\" ") or die("Cant branch get groups ".$db->error);
    ...
    function branch_members($branch){
    	//members of a branch
    	global $db;
    	$query = $db->query("SELECT * FROM members WHERE branchid = \"$branch\" ") or die("Can;t get branch members $db->error");
    	$members = array();
    ...
    function getChurchList(){
    	...
    	$query = $db->query("SELECT * FROM church") or trigger_error($db->error);
    ...
    function church_members($churchid){
    	...
    	global $db;
    	$query = $db->query("SELECT members.* FROM members JOIN branches ON members.branchid = branches.id WHERE branches.church = $churchid ") or die("Can't get members $db->error");
    	$members = array();
    	while ($data = $query->fetch_assoc()) {
    		$members[] = $data;
    	}
    ...
    function church_podcasts($church){
      ...
      $church = $db->real_escape_string($church);
      $query = $db->query("SELECT * FROM podcasts WHERE church =\"$church\" AND status = 'active' ") or die("Error getting church podcats $db->error");
      $pods = array();
    ...
    function addMessage($sender, $message, $channel, $subject, $scheduleTime="", $smsName=null){
        ...
        $query = mysqli_query($db, $sql) or die("Okay".$db->error);
    ...
    function mlogs($id){
        ...
        //Message logs of $id
        $query  = mysqli_query($db, "SELECT * FROM messageslog WHERE message = \"$id\"") or die(mysqli_error($db));
    ...
    function churchbranches($church){
        global $db;
        $query = $db->query("SELECT * FROM branches WHERE church = \"$church\" ") or die("Error getting church branches ".$db->db);
        $branches = array();
        while ($data = $query->fetch_assoc()) {
          $branches[] = $data;
        }
